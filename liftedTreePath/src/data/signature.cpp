...
USignature::USignature(const USignature& sig) : _name_id(sig._name_id), _args(sig._args), _unique_id(sig._unique_id), last_parent_method_id(sig.last_parent_method_id), shadow_action(sig.shadow_action), first_child_of_reduction(sig.first_child_of_reduction) {}
USignature::USignature(USignature&& sig) : _name_id(sig._name_id), _args(std::move(sig._args)), _unique_id(sig._unique_id), last_parent_method_id(sig.last_parent_method_id), shadow_action(sig.shadow_action), first_child_of_reduction(sig.first_child_of_reduction) {}
...
USignature& USignature::operator=(const USignature& sig) {
    ...
    shadow_action = sig.shadow_action;
    first_child_of_reduction = sig.first_child_of_reduction;
    _args = sig._args;
    return *this;
}
